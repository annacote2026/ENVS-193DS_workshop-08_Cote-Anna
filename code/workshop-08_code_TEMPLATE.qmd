...
```{r model-diagnostics}
# code to display plots in a 2x2 grid
par(mfrow = c(2, 2))
plot(model1)
plot(model2)
plot(model3)
plot(model4) # looks different because of categorical predictors
plot(model5)
plot(model6) # only predictor is categorical (water_treatment)
plot(model7)
...
AICc(
  ...
  arrange(AICc) # model4 has the lowest AICc (after weighing all combinations of predictors, the best model only includes water treatment and species; if you just know these you can predict what a plant's mass would be)
```
...
```{r model-summary}
summary(model4)
# reference level for water_treatment: drought stressed because it doesn't show up in the model summary
# reference level for species_name: lotus scoparius (these are the reference levels because they were the first categories you set in the code at the beginning)
```
...
This model explained 73% (adjusted R squared) of the variation in total plant mass.
...
```{r model-predictions}
model4_predictions <- ggpredict( # naming the model
  model4,
